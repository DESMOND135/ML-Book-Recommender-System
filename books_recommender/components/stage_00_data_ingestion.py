...
class DataIngestion:
    ...
    def download_data(self):
        """
        Fetch the data from URL or local path.
        If it's a local file path, skip download.
        """
        try:
            ...
            # If the URL is actually a local file path
            if os.path.exists(dataset_url):
                logging.info(f"Using local dataset: {dataset_url}")
                return dataset_url
